...
- 本文件只定义完整版输出形态，不承担命名、版本、发布门禁、接口选择或文件载体规则的长期定义；这些规则以 `references/rules/*.md` 为准。
...
- `Quick Draft` 仅用于首轮快速讨论，风险表只保留 `3..5` 个高优先级真实风险项加 `R-OTHER`。
- `Formal Draft` 为默认草稿模式，风险表应完整覆盖待确认和待修改项。
...
正式架构 Markdown 只承载软件架构设计正文；完整评审、检查和追溯闭环应作为同目录伴生产物输出。

| 产物 | 文件名 | 用途 |
| --- | --- | --- |
| Review 架构评审记录 | `Review_{FC}_软件架构设计.md` | 记录评审重点、release blocker、风险关闭记录、评审结论和 SDS 进入判断。 |
| Check 架构检查清单 | `Check_{FC}_软件架构设计.md` | 记录 Gate 检查项、检查结果、证据、主要问题和下一步动作。 |
| Trace 追溯矩阵 | `Trace_{FC}_软件架构设计.md` | 记录 SRS → Architecture 的覆盖对象、状态、架构落点、设计决策和关闭条件。 |

架构正文可以引用这些伴生产物，但不要在正文内重复完整 Review/Check/Trace 表。
