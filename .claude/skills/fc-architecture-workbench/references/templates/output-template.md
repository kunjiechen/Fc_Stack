# FC软件架构生成脚手架（内部使用，非交付模板）

> **定位警告：**
> 本文件是 skill **内部生成脚手架**，用于在生成过程中组织需求抽取、接口筛选、反向追踪、遗漏分析和候选接口判定等中间推理产物。
>
> **严禁将本文件直接作为交付模板加载。** 最终交付给用户的正式架构文档必须使用 `output-template-summary.md`。
>
> 本文件存在的唯一价值是：当 skill 需要在生成过程中对需求做全量抽取、逐一判定后，在这里汇总证据链和判定依据，再压缩映射到精简版模板输出。它不应该出现在用户的文件系统里。

说明：
- 本文件用于 skill 内部推理时组装完整的抽取证据、候选接口、配置清单、反向追踪和遗漏分析。
- 所有 `§0`（抽取与判定总览）、`§2`（设计输入展开）、`§3`（假设与缺失信息）、`§4`（需求到架构映射及反向追踪）、`§7.1`（接口统一性判定）、`§12`（命名检查及遗漏清单）都属于过程产物，不出现在最终交付文档中。
- 若需交付正式架构文档，必须将本脚手架中的结论压缩、重排、去过程化后，按 `output-template-summary.md` 的结构输出。
- 本文件只定义内部推理的承载形态，不承担命名、版本、发布门禁、接口选择或文件载体规则的长期定义；这些规则以 `references/rules/*.md` 为准。

## 模板加载规则

- **正式架构生成** → 加载 `output-template-summary.md`（默认）
- **架构评审 / 校验 / 问题排查** → 加载本脚手架，用于全量抽取证据和反向追踪
- **Quick Draft** → 仍使用 `output-template-summary.md`，风险表仅保留 3~5 条高优先级项
- **Formal Draft / Released** → 使用 `output-template-summary.md` 的完整形态
- **绝对不要**将本脚手架的任何过程性章节（§0/§2/§3/§4/§7.1/§12）写入用户交付文档
