...
- 外部接口和依赖接口的描述应优先使用英文。
- 本模板默认包含以下 10 个章节。
- 本文件只定义精简版输出形态，不承担命名、版本、发布门禁、接口选择或文件载体规则的长期定义；这些规则以 `references/rules/*.md` 为准。
...
- 若存在故障检测、诊断判定或异常状态上报，本节默认应包含一个可读取的故障/诊断状态接口，例如 `GetFaultStatus` 或 `GetDiag`。
...
说明：
- `Quick Draft`：仅保留 `3..5` 个最高优先级真实风险项，加 `R-OTHER`。
- `Formal Draft`：列出完整风险与待确认项，作为后续评审和发布依据。
